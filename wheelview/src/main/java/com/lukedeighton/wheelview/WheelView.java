...
public class WheelView extends View {
    ...
    private boolean dragable = true; // dragable by default
    ...
    public void initWheelView() {
        ...
        mItemTransformer = new SimpleItemTransformer();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = offset; i < length; i++) {
            ...
            if (Rect.intersects(sTempRect, mViewBounds)) {
                if (cacheItem.mDirty) {
                    cacheItem.mDrawable = mAdapter.getMenuItem(adapterPosition).getIcon();
                    cacheItem.mTitle = mAdapter.getMenuItem(adapterPosition).getTitle();
